...
export default function ManageReturnRequest() {
  ...
  useEffect(() => {
    handleSetDashboardTitle("Manage return requests");
  ...
  const fetchList = async () => {
    ...
    try {
      const res = await axios.get(
        "http://localhost:9999/api/admin/return-requests",
        {
          headers: {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(
        `http://localhost:9999/api/admin/return-requests/${selected._id}`,
        {
          status,
        },
        {
            ...
            <TableBody>
              ...
              {filteredRequests.map((row) => (
                <TableRow key={row._id}>
                  ...
                  <TableCell>
                    {row.userId?.fullname || row.userId?.username}
                  </TableCell>
              ...
              {selected.orderItemId ? (
                <Table size="small">
                  ...
                  <TableBody>
                    <TableRow>
                      <TableCell>
                        {selected.orderItemId.productId?.title || "(deleted)"}
                      </TableCell>
                      <TableCell>
