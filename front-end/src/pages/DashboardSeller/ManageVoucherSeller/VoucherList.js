...
const VoucherSellerList = () => {
  ...
  const fetchVouchers = async () => {
    try {
      ...
      if (status === 403) {
        displayMessage =
          "You do not have permission to access vouchers. Please make sure you are logged in as an admin.";
      } else if (status === 401) {
  ...
  const handleSubmitForm = async (formData) => {
    try {
      ...
      setSnackbar({
        open: true,
        message: `Error: ${
          error.response?.data?.message || "Could not save voucher"
        }`,
        severity: "error",
      });
    } finally {
        ...
        <CardContent>
          ...
          {vouchers.length === 0 && !loading ? (
            <Alert severity="info" sx={{ my: 2 }}>
              No vouchers have been created yet. Add a new voucher using the
              "Add Voucher" button.
            </Alert>
          ) : (
            <TableContainer
                  ...
                  {filteredVouchers.map((voucher) => (
                    <TableRow
                      ...
                      sx={{
                        ...
                        backgroundColor: isExpired(voucher.expirationDate)
                          ? "rgba(0,0,0,0.04)"
                          : "inherit",
                      }}
                    >
                      ...
                      <TableCell align="center">
                        <IconButton
                          size="small"
                          onClick={(event) =>
                            handleOpenActionMenu(event, voucher)
                          }
                        >
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>
            ...
            <b>{deleteDialog.voucher?.code}</b>? This action cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button
...
export default VoucherSellerList;
