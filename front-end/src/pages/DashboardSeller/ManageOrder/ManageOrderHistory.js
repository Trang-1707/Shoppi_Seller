...
export default function ManageOrderHistory() {
  ...
  const handleUpdateStatus = async () => {
    ...
    try {
      ...
      const response = await api.put(
        `seller/orders/item/${selectedOrderItem._id}/status`,
        {
          ...
          return {
            ...
            shippingInfo:
              statusToUpdate === "shipping"
                ? response.data.data.shippingInfo
                : item.shippingInfo,
          };
        }
        return item;
  ...
  const handleUpdatePaymentStatus = async () => {
    ...
    try {
      setLoadingAction(true);
      const response = await api.put(
        `seller/payments/${orderPayment.payment.id}/status`,
        {
      ...
      if (paymentStatusToUpdate === "paid") {
        const updatedItems = orderItems.map((item) => {
          if (
            item.orderId._id === orderPayment.order.id &&
            item.status === "pending"
          ) {
            return {
              ...item,
      ...
      // Show success message
      setSnackbarMessage(
        `Payment status successfully updated to ${
        ...
        <>
          ...
          <Card
            sx={{
              ...
              boxShadow: 3, // Bóng đổ nhẹ nhàng hơn
              zIndex: 1400,
            ...
            <CardContent>
              ...
              <Stack spacing={2}>
                ...
                <Stack direction="row" spacing={2} alignItems="center">
                  <Avatar
                    sx={{
                      bgcolor: theme.palette.success.light, // Màu sáng hơn
                      color: theme.palette.success.dark, // Icon màu tối hơn
                      width: 48, // Kích thước lớn hơn một chút
                      height: 48,
                    }}
                  >
                    <LocalShippingIcon />
                ...
                <Stack direction="row" spacing={2} alignItems="center">
                  <Avatar
                    sx={{
                      bgcolor: theme.palette.warning.light, // Màu sáng hơn
                      color: theme.palette.warning.dark, // Icon màu tối hơn
                      width: 48, // Kích thước lớn hơn một chút
                      height: 48,
                    }}
                  >
          ...
          >
            <MenuItem
              onClick={() => handleStatusOptionClick("shipping")}
              sx={{ py: 1 }}
            >
              <LocalShippingIcon
                fontSize="small"
            ...
            <MenuItem
              onClick={() => handleStatusOptionClick("rejected")}
              sx={{ py: 1 }}
            >
              <CancelIcon
                fontSize="small"
          ...
          >
              ...
              {selectedOrderItem && (
                ...
                >
                  <Typography variant="subtitle2" display="block" gutterBottom>
                    ...
                    {selectedOrderItem.productId?.title}
                  </Typography>
                  ...
                  <Typography variant="body2" display="block">
                    <strong>Price:</strong> $
                    {selectedOrderItem.unitPrice?.toLocaleString()}
                  </Typography>
                </Box>
              )}
            </DialogContent>
            <DialogActions sx={{ px: 3, pb: 2 }}>
              ...
              <Button
                ...
                disabled={loadingAction}
                startIcon={
                  loadingAction ? <CircularProgress size={16} /> : null
                }
              >
                {loadingAction
                  ? "Processing..."
                  ...
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      <Typography variant="body1">
                        ...
                        {orderPayment.payment.amount.toLocaleString()}
                      </Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12}>
                    ...
                    >
                      ...
                      {orderPayment.payment.paidAt && (
                        <Typography variant="body1">
                          ...
                          {new Date(orderPayment.payment.paidAt).toLocaleString(
                            "en-US",
                            {
                              year: "numeric",
                              month: "short",
                      ...
                      <Box sx={{ mt: 1 }}>
                        <Tooltip title="Once marked as paid, status cannot be changed back">
                          <Button
                            ...
                            startIcon={<MoneyIcon />}
                            onClick={() =>
                              handleOpenPaymentConfirmDialog("paid")
                            }
                            sx={{ mr: 2 }}
                          >
                            Mark as Paid
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleOpenPaymentConfirmDialog("failed")
                          }
                        >
                          Mark as Failed
                        </Button>
              ...
              <Typography>
                ...
                {paymentStatusToUpdate === "paid" && (
                  <Typography color="error" sx={{ mt: 1 }}>
                    Warning: Once marked as paid, you cannot change this status
                    back!
                  </Typography>
                )}
              </Typography>
